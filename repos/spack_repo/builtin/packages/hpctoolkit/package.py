...
class Hpctoolkit(AutotoolsPackage, MesonPackage):
    ...
    version("2025.0.0-rc.1", tag="2025.0.0-rc.1", commit="fcdf9e9f8272e4a3783e014e8446c3bcea6a22b1")
    ...
    variant(
        "auditor-default",
        default=True,
        description="When true, use LD_AUDIT in hpcrun by default",
        when="@2025:",
    )
    variant(
        "debug",
...
class MesonBuilder(meson.MesonBuilder):
    def meson_args(self):
        ...
        if spec.satisfies("@2025:"):
            args.append("-Dhpcrun_use_auditor_by_default=" + (
                "true" if spec.satisfies("+auditor-default") else "false"))
